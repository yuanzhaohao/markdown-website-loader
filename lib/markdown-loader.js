...
module.exports = function(source) {
  ...
  console.log(this.resourceQuery, this.request);
  ...
  if (!params.isDemo) {
    ...
    return callback(null, transformDemo(source));
  }
};

function transformCommon(source, demoPath) {
  const mdFiles = glob.sync(path.join(demoPath, './*.md'));
  return (
    `module.exports = {` +
    `\nmarkdown: ${JSON.stringify(md.render(source))},` +
    ...
    mdFiles
      .map(filepath => {
        console.log(filepath, loaderUtils.stringifyRequest(this, `${filepath}?+isDemo`));
        return (
          `'${path.basename(filepath, '.md')}': require(` +
          loaderUtils.stringifyRequest(this, `'${filepath}?isDemo=1'`) +
          // `'${filepath}?+isDemo'` +
          `)`
        );
      })
